...
function Home() {
  return (
    <div className="relative bg-black min-h-screen text-white font-sans">
      ...
      <section className="relative z-10 bg-[#0a0a0a] py-16 sm:py-20 lg:py-24 border-t border-gray-900">
        <div className="container mx-auto px-5 sm:px-8 lg:px-10">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-10 sm:mb-12 lg:mb-16">
            <div>
              <h4 className="text-blue-500 font-bold uppercase tracking-widest text-xs sm:text-sm mb-2">Capabilities</h4>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-light">What We Do Best</h2>
            </div>
            <button className="text-gray-400 hover:text-white border-b border-gray-800 pb-1 text-sm sm:text-base">View All Services</button>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {Object.keys(servicesData).slice(0, 8).map((key) => (
              <div key={key} className="p-5 sm:p-6 lg:p-8 border border-gray-900 hover:border-blue-500 transition-all duration-500 group bg-black/40">
                <div className="text-3xl sm:text-4xl mb-4 sm:mb-6 grayscale group-hover:grayscale-0 transition-all">{servicesData[key].icon || '🚀'}</div>
                <h3 className="text-xl sm:text-2xl font-medium mb-3 sm:mb-4">{servicesData[key].title}</h3>
                <p className="text-gray-500 text-sm sm:text-base line-clamp-3 mb-4 sm:mb-6">{servicesData[key].description}</p>
                <button className="text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity text-sm sm:text-base">Learn More →</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto px-5 sm:px-8 lg:px-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-10 lg:gap-12 mb-12 sm:mb-16 lg:mb-20">
            ...
            <div>
              ...
              <ul className="text-gray-500 space-y-3 sm:space-y-4 text-sm sm:text-base">
                <li>Hamindpur, Alwar, Rajasthan</li>
          ...
          <div className="border-t border-gray-900 pt-8 sm:pt-10 flex flex-col md:flex-row justify-between items-center text-gray-600 text-xs sm:text-sm gap-4 md:gap-0">
            <p>© 2026 Webraahi & Aurix AI. All rights reserved.</p>
